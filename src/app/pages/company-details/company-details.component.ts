...
export class CompanyDetailsComponent implements OnInit {
  ...
  company: Company;
  firmType: any[] = [ {value: 0, name: "Kurumsal"}, {value: 1, name: "Bireysel"} ];
  public provinceList: any[];
  districtList: any[] = [];

  constructor(private companyService: CompanyService, private sharedService: SharedService, private messageService: MessageService) { 
    this.getCompany();
  }

  ngOnInit(): void {
    this.company = new Company();
    this.provinceList = dataProvince.filter(provinceList => provinceList);
  }

  getCompany() {
    this.companyService.getCompany(this.sharedService.getCompanyId()).subscribe(company => {
  ...
  saveCompany() {
    var tempProvince: number = +((document.getElementById("input-province")as HTMLInputElement).value);
    var tempDistrict: number = +((document.getElementById("input-district")as HTMLInputElement).value);
    this.company.provinceId = tempProvince;
    this.company.districtId = tempDistrict;

    if (!this.company.companySector && !this.company.companyTitle && !this.company.companySector) { return; }
  ...
  getDistrict(): void {
    const selectedProvince = this.company.provinceId;
    this.districtList = dataDistrict.filter(district => {
      return district.il_id == selectedProvince.toString();
    });
  }
  ...
  handleUploadProfileImage() {
    ...
    fileInput.addEventListener("change", () => {
      ...
      const image = document.querySelector(".card-profile-image img") as HTMLImageElement;
      image.src = fileUrl;
    });
  }
